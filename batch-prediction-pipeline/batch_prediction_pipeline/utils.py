...
from batch_prediction_pipeline.settings import OUTPUT_DIR, SETTING
...
def get_bucket(
    ...
    bucket_project: str=SETTING["GOOGLE_CLOUD_PROJECT"],
...
def write_blob_to(bucket: storage.Bucket, blob_name: str, data: pd.DataFrame):
    """Write a dataframe to a GCS bucket as a csv file.
    ...
    try:
        ...
        with blob.open("wb") as f:
            data.to_csv(f)
        print(f"Dataframe successfully written to {blob_name} in the bucket.")
    except Exception as e:
        print(f"Error writing dataframe to {blob_name}: {str(e)}")
