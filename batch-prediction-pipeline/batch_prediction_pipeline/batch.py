...
from batch_prediction_pipeline import data, transformers
...
def save(X: pd.DataFrame, y: pd.DataFrame, predictions: pd.DataFrame):
    ...
    for df, blob_name in zip(
        [X, y, predictions], ["X.csv", "y.csv", "predictions.csv"]
    ):
...
def save_for_monitoring(predictions: pd.DataFrame, start_datetime: datetime):
    ...
    cached_predictions = utils.read_blob_from(
        bucket=bucket, blob_name=f"predictions_monitoring.csv"
    )
    ...
    if has_cached_predictions is True:
        ...
        cached_predictions["datetime_utc"] = pd.PeriodIndex(cached_predictions["datetime_utc"], freq="H")
        cached_predictions = cached_predictions.set_index(["area", "consumer_type", "datetime_utc"]).sort_index()

        
        cached_predictions.index = cached_predictions.index.set_levels(
            cached_predictions.index.levels[2], level=2)
    ...
    utils.write_blob_to(
        bucket=bucket,
        blob_name=f"predictions_monitoring.csv",
        data=predictions,
    )
